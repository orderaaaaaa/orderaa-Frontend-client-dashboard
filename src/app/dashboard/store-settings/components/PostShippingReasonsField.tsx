...
import { LiaTimesSolid, LiaUndoAltSolid } from 'react-icons/lia';
import {
  usePostShippingReasons,
  useCreatePostShippingReason,
...
export function PostShippingReasonsField() {
  ...
  const { data: reasons, isLoading } = usePostShippingReasons();
  const { mutate: createReason, isPending: isCreating } =
    useCreatePostShippingReason();
  const { mutate: deleteReason } = useDeletePostShippingReason();
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-start gap-3">
        <LiaUndoAltSolid className="w-5 h-5 sm:w-6 sm:h-6 text-primary mt-0.5 shrink-0" />
        <div className="flex-1 min-w-0">
          <h3 className="text-sm sm:text-base lg:text-lg font-semibold text-gray-900">
            حالات رفض الاستلام / المرتجع بعد الشحن
          </h3>
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
            أضف الأسباب التي تظهر عند إلغاء أو إرجاع الطلب بعد خروجه لشركة
            الشحن
          </p>
        </div>
      </div>
