...
function PageTaps({ data, totalCount = 0, onStatusChange, activeStatus }: PageTapsProps) {
  return (
    <div className="flex gap-4 md:flex-wrap overflow-x-auto hide">
      ...
      <PageTab
        ...
        active={activeStatus === 'pending_payment'}
      />
      <PageTab
        label="واتساب"
        count={0}
        ...
        onClick={() => onStatusChange?.('whatsapp')}
        active={activeStatus === 'whatsapp'}
      />
      <PageTab
        label="تأجيلات"
        count={0}
        ...
        active={activeStatus === 'postponed'}
      />
      <PageTab
        label="اعادة اتصال"
        count={0}
        icon={<PhoneCall width={18} height={18} />}
        onClick={() => onStatusChange?.('recall')}
        active={activeStatus === 'recall'}
      />
      <PageTab
        label="وقوف التشغيل"
        count={0}
      ...
      <PageTab
        ...
        onClick={() => onStatusChange?.('prepared')}
        active={activeStatus === 'prepared'}
      />
      <PageTab
        label="في الشحن"
        count={0}
      ...
      <PageTab
        ...
        active={activeStatus === 'collected'}
      />
      <PageTab 
        label="تسليم جزئي" 
        icon={<FileScan width={18} height={18} />}
        onClick={() => onStatusChange?.('partial_delivery')}
        active={activeStatus === 'partial_delivery'}
      />
      <PageTab 
        label="طلبات مفقوده" 
