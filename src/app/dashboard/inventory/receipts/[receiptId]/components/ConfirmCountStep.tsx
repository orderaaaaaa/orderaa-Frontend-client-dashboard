...
function getProductStatus(
  ...
  for (const v of variants) {
    const scanned = scannedCounts[v.id] ?? 0;
...
const ConfirmCountStep = memo(({ productVariants }: ConfirmCountStepProps) => {
  ...
  const incrementVariant = useCallback(
    (variantRowId: string) => {
      setScannedCounts((prev) => ({
        ...prev,
        [variantRowId]: (prev[variantRowId] ?? 0) + 1,
      }));
  ...
  const columns: DataTableColumn<ConfirmVariantRow>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        className: 'w-28',
        render: (_value: unknown, row: ConfirmVariantRow) => {
          const scanned = scannedCounts[row.id] ?? 0;
    ...
    [scannedCounts, lastScannedVariant, incrementVariant]
  );
  ...
  const overallTotals = useMemo(() => {
    ...
    for (const v of allVariants) {
      totalScanned += scannedCounts[v.id] ?? 0;
      totalExpected += v.expectedQuantity;
    }
    return { totalScanned, totalExpected };
  }, [productsWithVariants, scannedCounts]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {productsWithVariants.map(({ product, variants }) => {
        const productStatus = getProductStatus(variants, scannedCounts);
