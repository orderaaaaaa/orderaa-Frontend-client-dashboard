  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-6 w-[95%]">
      {SummaryCardConst.map(({ id, data, title, desc, percentage }) => (
        <div
          key={id}
          className="flex flex-col gap-2 p-8 border-2 min-w-[300px] bg-[#f9f9fb] border-[#5D24E180] rounded-3xl shadow-lg w-full"
        >
          <h1 className="text-[#5D24E1] font-medium text-[25px]">{title}</h1>
