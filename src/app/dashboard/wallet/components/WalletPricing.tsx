...
function WalletPricing() {
  return (
    <section id="pricing" className="mt-13 sm:px-4">
      <h2 className="text-2xl font-bold mb-8">اسعار شحن المحفظة</h2>
      ...
      <div className="container grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
        {WALLET_PRICING_PLANS.map((plan, index) => (
          <div
            key={index}
            className={`relative rounded-2xl p-9 border min-h-[420px] flex flex-col transition-transform duration-300 hover:scale-105 overflow-hidden text-right hover:border hover:border-primary
              ${
                plan.gradient
