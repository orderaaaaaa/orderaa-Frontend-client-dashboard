...
export default function ActionConfirmationDialog({
  ...
  const handleConfirm = async () => {
    try {
      // Disable button immediately when starting the API call
      setIsLoading(true);

      // Call onConfirm and wait for it to complete
      await onConfirm();

      // Only show success animation if onConfirm succeeded
      setShowSuccess(true);
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-[500px] bg-white rounded-[20px] shadow-xl transition-all duration-300 ${showSuccess ? 'scale-95 opacity-90' : 'scale-100 opacity-100'
          }`}
